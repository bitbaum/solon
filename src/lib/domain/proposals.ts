...
/**
 * File a proposal. Either a signed-in human member files it with one click
 * (account), or the proposer signs it (key): then the proposer is whoever the
 * signature recovers to, and for policy proposals the signature binds the
 * sha256 of the exact proposed content — nothing can be swapped after signing.
 * Agents may propose in ANY category (the electorate only gates voting), but
 * always sign and must additionally present their transport API key.
 */
export async function createProposal(input: CreateProposalInput): Promise<CreateProposalResult> {
  ...
  if (by.via === "key") {
    const message = proposalMessage({
      ...
      title: input.title,
      proposerAddress: by.address,
      contentHash,
      optionsHash,
    });
    const verification = verifyMessage(message, by.address, by.signature);
  ...
  if (by.via === "account" && member.memberType !== MemberType.HUMAN) {
    return { created: false, verified: true, reason: "agent members propose with their key" };
  }
